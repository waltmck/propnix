...
pkgs.lib.makeScope pkgs.newScope (
  ...
  with self;
  {
    ...
    wine = callPackage ../emulators/wine-hangover.nix { }; # Hangover fork; arm64ec archs on aarch64, plain on x86_64
    ...
    propnix-prefetch = callPackage ../pkgs/propnix-prefetch { }; # cold-launch cache warmer (posix_fadvise)
    propnix-launcher = callPackage ../pkgs/propnix-launcher { }; # the per-app launcher (splash + seal + orchestration)

    # ── lib helpers (arch-agnostic) ──
    sealing = import ./sealing.nix { inherit (pkgs) lib; }; # §7 env-seal data model (pure lib, not a derivation)
    winefexDefaults = import ./winefex-defaults.nix; # global tuning base every game inherits (§5/§6)
    ...
    # ── games ──
    hollow-knight = callPackage ../pkgs/games/hollow-knight { }; # Windows title; same payload on both arches
  }
