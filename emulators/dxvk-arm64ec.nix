...
# baked into ./llvm-mingw (RESEARCH §22).
#
#   nix build .#dxvk   # -> $out/{d3d11,d3d10core,dxgi,d3d9,d3d8}.dll (all 0xA641)
{
  stdenv,
  writeText,
  meson,
...
let
  ...
  # because the EC builtins archive is fixed in place inside llvmMingw (llvm-mingw postFixup).
  crossFile = writeText "dxvk-arm64ec-cross.txt" ''
    [binaries]
...
stdenv.mkDerivation {
  pname = "dxvk-arm64ec";
  version = "2.7.1";
  src = dxvk_2.src; # pinned DXVK 2.7.1 (same source nixpkgs uses for the x86 PE build)

  nativeBuildInputs = [
    meson
    ninja
    pkg-config
    glslang # glslangValidator — compiles DXVK's shaders (runs native on the build host)
    python3
    llvmMingw
  ];
