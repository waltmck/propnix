# Kerbal Space Program — per-title tuning. Layered over lib/wine-defaults.nix (sealing.mergeTuning), so
# this states only what is SPECIFIC to KSP. A FUNCTION of `payload` (makeAppWine calls it) because KSP's
# game-dir overlay references the payload's store path directly.
{ payload }:
{
  ...
  # persistent state. (propnix-mount implements source-less-tmpfs + seed; see lib/makeAppWine.nix / mount docs.)
  ...
  # the saves/ of) the whole-gamedir overlay above.
  ...
  # BROKEN on aarch64 (see default.nix `brokenSystems`): with the writable Managed tmpfs above, KSP passes
