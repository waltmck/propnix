# Iron Lung — per-title tuning. Layered over lib/wine-defaults.nix (sealing.mergeTuning), so this states
# only what is SPECIFIC to Iron Lung (David Szymanski). Small Unity submarine-horror title (D3D11 →
# DXVK → Vulkan). A FUNCTION of `payload` (makeAppWine calls it) because the Managed-assemblies seed
# references the payload's store path directly.
{ payload }:
{
  ...
  mounts."drive_c/users/propnix/AppData/LocalLow/David Szymanski/Iron Lung" = {
