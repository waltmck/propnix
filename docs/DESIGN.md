...
**fex** (`backends/fex/default.nix`): `modules = []` (reuses `box64.*`); guest-only library union from `pkgsX86`; the `PT_INTERP`-patched executables as an `extraLowers` overlay with `executables = []` — via the shared `mkPatchedExes` (builders/patched-exes.nix), which the native face uses too, and which takes each executable from the highest-priority game tree that actually provides it; `FEX_ROOTFS = "/"`; `brokenSystems` for the 16K walls (D12).
