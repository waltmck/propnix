...
#
# Build/verify:
#   nix build .#fexdlls   # -> $out/{libarm64ecfex.dll,libwow64fex.dll,wowbox64.dll}
...
let
  ...
  wowbox64 = stdenv.mkDerivation {
    ...
    nativeBuildInputs = [
      cmake
      ninja
      python3 # functions_list (wrapper table) + gen_dynacache_hashes.py + override_wine_builtin.py
      git # generate_git_head_target runs `git rev-parse` (no .git here → empty GITREV, harmless)
...
stdenv.mkDerivation {
  ...
  nativeBuildInputs = [
    cmake
    ninja
    python3 # find_package(Python 3.9 REQUIRED) — configure-time only
    coreutils # patch_library_wine() runs `dd` as a POST_BUILD step on each DLL
