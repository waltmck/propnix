...
| `PROPNIX_STEAM_ACCOUNT` | which stored Steam account `propnix pin` / `propnix hash` use when several are stored (flag beats env beats try-all). Not read at launch time — the gbe_fork offline-entitlement shim manages its own identity. |
