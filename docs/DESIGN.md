...
- box86 as the 32-bit x86 Linux route on aarch64. Dead on 16K pages. 32-bit x86 *Linux* titles are served by `FEXInterpreter` instead (`emulators/fex-linux`, the `fex` backend — see D12), and 32-bit *Windows* titles by the wine engine's WoW64 path with `wowbox64`.
...
**D4 — Backend selection is one pure function; empirical failures are data, not types.** `resolveStrategy` (`lib/strategy.nix`, which also owns the platform table) maps the payload's `{ os; arch; }` × the host system to `wine`/`box64`/`fex`/`native` and serves only as the **default** of the typed `backend` option. The two x86 Linux widths take different arms on aarch64 — `x86_64` → box64, `i386` → fex, because box64 emulates x86_64 only — and both run as `native` on an x86_64 host. It is **total over the registry**: it answers which backend *would* run the content, never whether this host can, because a sentinel would leak into the `backend` enum and turn "this host cannot run it" into an eval error — which the CI matrix (one forced `.apply` per pinned pair, on **both** systems) would report as a red leg. Host capability is the separate predicate `runnable`, which the `emulatedPlatform` resolver uses to skip platforms this host cannot execute (`aarch64-linux` on x86_64: no ARM-on-x86 emulator) and which the thin backend turns into `meta.broken` for the explicit-override path — a BUILD refusal, never an evaluation failure — `.apply { backend = "fex"; }` still wins, and the selection logic stays a unit-testable function outside the module fixpoint. It selects the *emulator for an already-selected payload variant*; which build to run is the game module's judgment, stated as `mkDefault`s on the two axes (Stellaris: the Steam Linux build under box64, because the Windows build hangs under FEX/ARM64EC). Empirical failures (KSP FEX codegen; wine/FEX walls) are caught by `broken.systems` + the verified manifest, never by a type system — a route table would be pure maintenance surface with near-zero defect-catching power.

**D5 — `pkgsX86` is a lazy scope attribute; combining its outputs runs natively.** In `lib/default.nix`: identity on x86_64 (zero second eval); on aarch64 a second native instantiation of the **same pinned** nixpkgs (`import pkgs.path { system = "x86_64-linux"; … }`), yielding **substitutable** prebuilt Hydra x86_64 `.so`. **Invariant:** any derivation that *combines* `pkgsX86` outputs is built with the **native** `pkgs` builders taking `pkgsX86` packages as *inputs* — never a `pkgsX86` build step, which would demand qemu/binfmt the Asahi host does not run. `lib.makeLibraryPath` over `pkgsX86` packages is pure eval and always safe. `pkgsX86` is only ever a build-time dependency; every derivation stays host-arch. Its 32-bit sibling `pkgsGuest32` (for an `i386-linux` payload) takes a DIFFERENT route per host, because the same reasoning points the other way: `pkgsi686Linux` on x86_64 (a native 32-bit set an x86_64 CPU executes and Hydra caches), but `pkgsCross.gnu32` — CROSS-compiled — on aarch64, where a native i686 instantiation would build every uncached path under qemu/binfmt and be unbuildable without a substituter. Same invariant: guest outputs are inputs, never build steps.
...
**D7 — Per-game library declarations are resolved twice; the union cannot drift.** The box64 backend's option namespace (`box64.bridgingLibs`/`box64.guestLibs`, declared in `lib/backends/box64/options.nix`) holds `pkgs -> [drv]` functions. On aarch64 the entry resolves `bridgingLibs` against **both** `pkgs` and `pkgsX86` (box64 needs the native `.so` to wrap *and* the x86_64 copy for the guest) plus `guestLibs pkgsX86`; on x86_64 it collapses to one native set. The FEX backend **reuses** the same `box64.*` declarations as a pure guest union (no bridging), resolved from the set matching the payload's ABI — `pkgsX86` for an `x86_64-linux` payload, `pkgsGuest32` for an `i386-linux` one; the native face does the same, which is what lets an x86_64 host run a 32-bit payload with the 32-bit loader and 32-bit sonames instead of silently handing it the host's. One declaration, per-backend resolution — the original Hollow Knight wrapper's #1 hazard (native/guest lists drifting) is structurally impossible.
...
**D12 — Two FEX mechanisms, never conflated; both ship.** (A) **FEX ARM64EC DLLs inside wine** (`HODLL64=libarm64ecfex.dll`; 32-bit via `wowbox64.dll`, box64's `-DWOW64=ON` build) — the Hangover backend for Windows guests on aarch64, entirely inside the wine engine. (B) **`FEXInterpreter`** for x86 *Linux* guests of EITHER width — the box64 sibling, `lib/backends/fex/default.nix`: it reuses `box64.*` as a pure guest union (FEX bridges nothing), patches the declared executables' `PT_INTERP` to the guest glibc loader in a tiny overlay above the read-only payload, stages FEX's own GL/EGL guest thunks under their sonames (so the GPU driver runs host-side and no x86 Mesa is emulated), and sets `FEX_ROOTFS=/` so absolute store paths resolve as guest files. **x87 precision is an app-wide knob, not a backend detail:** `x87ReducedPrecision` (app-options, default TRUE) reaches every backend in its own spelling — `FEX_X87REDUCEDPRECISION` on fex, `BOX64_X87_NO80BITS` on box64's emulated face, and BOTH in an aarch64 wine prefix, which cannot know in advance whether it will load FEX's ARM64EC DLLs (x86_64 guest) or box64's wowbox64 (i386 guest). It is the single largest cost of an i386 guest, ahead of the JIT and far ahead of the thunks: the i386 ABI computes FP in the x87 stack, ARM64 has no 80-bit type, so the emulator falls back to software — `extF80_sub` measured 14.3% SELF time on Civ V while the entire host GL stack was 0.04%, and computing x87 in 64-bit doubles instead took its main menu from 59.8% to 24.0% of a core. The accuracy trade is real (both emulators default to precise) and one `x87ReducedPrecision = false` turns it off everywhere at once. Everything width-dependent — guest package set (`pkgsX86` / `pkgsGuest32`), loader, thunk directory — hangs off `cfg.emulatedPlatform`, not the host. **The emulator is ours, not nixpkgs':** stock FEX assumes a 4 KiB kernel page and will not start on a 16 KiB host, so `emulators/fex-linux` carries the large-host-page work plus two 32-bit-guest fixes (a 4 KiB-granular `MAP_FIXED` address in `64BitAllocator.cpp`, which only 32-bit guests reach; sub-host-page mmap emulation on the 32-bit syscall path) and an `installCheck` that runs all four guest classes. (B) is the ONLY route for a 32-bit x86 Linux payload — box64 emulates x86_64 only and box86 is dead on 16K — and `resolveStrategy` therefore sends `i386-linux` there by default (pkgs/games/civilization-5 is the title that forced it into existence, and it runs). For an `x86_64-linux` payload box64 remains the default; FEX is reachable per-game with `.apply { backend = "fex"; }`. The entry carries `brokenSystems = [ "x86_64-linux" ]`, because FEX emulates x86 *on ARM64* and has no x86_64-host build — and that is the ONLY refusal the backend makes. Whether FEX can carry a given title is a property of the title's ENGINE: a Mono/JIT engine additionally needs FEX's self-modifying-code tracking, which is 4 KiB-granular against a 16 KiB host page, and `pkgs/games/hollow-knight` records that crash in its own module as `broken.systems = lib.optionals (config.backend == "fex") [ "aarch64-linux" ]`. Per-title data, not a backend-wide type (D4) — a backend-wide refusal would be false for every JIT-free engine, Civ V's included.
...
│   │   ├── fex/default.nix        #   reuses box64.*; PT_INTERP patch + GL thunk + FEX_ROOTFS=/; i386|x86_64 guests
...
```nix
# need = { os = "windows"|"linux"; arch = "x86_64"|"i386"|"aarch64"; }; system = the host
# → "wine" | "box64" | "native"   (TOTAL over the registry — see D4)
resolveStrategy = need: system:
  if need.os == "windows" then "wine"          # wine picks native vs ARM64EC+HODLL (and the 32-bit DLL) internally
  else                                         # box64 is the one emulator here, and it emulates x86 only
    (if isX86Guest need && hostArchOf system == "aarch64" then "box64" else "native");

# Can this host run that content at all? Separate from WHICH backend would.
runnable = platform: system:
  need.os == "windows" || isX86Guest need || need.arch == hostArchOf system;
```

Pure and outside the module fixpoint; `resolveStrategy` is used only as the `backend` option's default, so
`.apply { backend = … }` wins (D4). `runnable` filters the `emulatedPlatform` resolver's walk and feeds the
thin backend's `meta.broken` contribution — so a game may rank a host-specific native build first
(factorio: `aarch64-linux`) without stranding the other host, which falls through the same ranking.
...
**`mkThinApp`** (thin.nix) — resolved config + launch block → THIN-mode package. Owns everything emulator-independent: the read-only game-tree union (`payloads` — enabled DLC first, then the depots, leftmost wins; the non-game `extraLowers` above all of them), ONE metacopy exec-bit skeleton per game tree stacked in that tree's own position (so a DLC shipping its own build of the game is never shadowed by the base's), `maskFiles` whiteouts, save binds, icon, splash/single-instance/window-watcher flags, and the `mode = "thin"` config JSON. `scrubPrefixes` defaults to `sealing.defaultScrub` — the same constant the wine seal uses.
...
4. **FEX SMC tracking on 16K.** The Mono/JIT wall (D12) is upstream; it blocks a JIT engine under the thin FEX backend, not the backend itself (Civ V ships on it). *Experiment:* re-run `hollow-knight.apply { backend = "fex"; }` on each FEX release (or any 4K host); a mapped, rendered window retires the wall by deleting that title's `broken.systems`/`broken.reason` lines.
