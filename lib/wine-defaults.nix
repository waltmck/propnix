# Global wine-app tuning defaults — the base every propnix wine app inherits (PLAN2 §5/§6), on BOTH the
# emulated aarch64 (winefex) and native x86_64 paths. A per-game `tuning.nix` layers on top via
# `sealing.mergeTuning` (per-game wins; `dllOverrides`/`mounts` merge per-key), so a game spec states only
# what is SPECIFIC to it. Each scalar knob is `{ value; reason; }` so a non-default choice justifies itself.
#
# Shape: `{ lib, prefixLower, galaxyStub ? null }: config: { … }`. The OUTER args are the store-path
# derivations its mount table references (the read-only `prefixLower` system tree; the GOG Galaxy no-op stub
# tree) — interpolated into mount rows directly at build time, with no runtime env indirection (and no way for
# a stray env var to redirect a source). The INNER `config` is the RESOLVED fixed-point tuning: this layer is
# a MODULE-SYSTEM config-function, so mount rows derived from other knobs (`galaxyStubDlls`, `extraSystem32`)
# recompute when those knobs are overridden (`sealing.resolveTuning` calls this with the final config).
# Exposed as the scope attr `wineDefaults`; override it to re-base all games.
{
  lib,
  prefixLower,
...
let
  ...
  # `overrideTweaks` that changes `galaxyStubDlls`/`extraSystem32` re-derives these rows. ──────────────────
  ...
  staticMounts = {
    ".update-timestamp" = { source = "${prefixLower}/.update-timestamp"; mode = "ro"; };
    "drive_c/windows" = { source = "${prefixLower}/drive_c/windows"; mode = "ro"; };
    ...
    "drive_c/Program Files" = { source = "${prefixLower}/drive_c/Program Files"; mode = "ro"; };
    "drive_c/Program Files (x86)" = { source = "${prefixLower}/drive_c/Program Files (x86)"; mode = "ro"; };
    # DOS drive mapping (c: → ../drive_c, z: → /): wine's own relative symlinks, bound read-only from the store.
    "dosdevices" = { source = "${prefixLower}/dosdevices"; mode = "ro"; };
...
in
{
  ...
  # ── The rest of a game's declarative CONFIG (moved here from makeAppWine's arg set so `tuning` is the single,
  # uniformly-layered home for everything about HOW an app runs; makeAppWine's own args are just identity
  # (pname/appid/name), content (payload), entry point (exe), and build-time packaging (icon, brokenSystems)).
  # Each has an inert default so a game states only what it needs, and each is reachable by `overrideTweaks`.

  # Baked command-line args passed to `exe` on every launch (before any runtime `-- <args>` passthrough) — for
  # per-game engine flags, e.g. a Unity title's `[ "-screen-fullscreen" "1" ]`. A game REPLACES this list.
  exeArgs = [ ];
  ...
  # DERIVED into the mount table here, so an `overrideTweaks` on this list re-derives the rows) binding the
  ...
  #
  # NOTE ON OVERLAYS: a store-backed overlay works unprivileged via the DATA-ONLY `skeleton` (makeAppWine
  ...
  # injected by makeAppWine, not declared here: the root is a persistent MOUNT of `$PROPNIX_STATE/wine/prefix`
  ...
  # default skeleton (makeAppWine) makes those root-owned lowers copy-up-able unprivileged, so writes persist
