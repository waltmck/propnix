# desktop-item.nix — mkPropnixDesktopItem: a `.desktop` launcher (+ symbolic icon) for a propnix app (§5).
#
...
# theme (extract-pe-icon.nix, merged into the app by makeAppWine); this builder just references it by name
# via `Icon=` and, if given, installs the monochrome `-symbolic.svg` variant (for DE contexts that prefer
# symbolic icons). `startupWMClass` ties the wine-created window to this entry (wine sets the class to the
# exe basename).
{
  lib,
  makeDesktopItem,
