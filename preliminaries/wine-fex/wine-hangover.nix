...
#   2. `arm64ec` added to --enable-archs (→ ARM64X-hybrid core DLLs; the fork defaults to arm64ec too).
#   3. `dontStrip = true` (§ below) — the ARM64X load-config must survive.
...
#
# dontStrip rationale: stripping the ARM64X PE DLLs (ntdll/kernel32/…) drops sections and invalidates the
# load-config's DynamicValueRelocTableSection index, so wine's update_arm64x_mapping() never rewrites the
# header machine ARM64->AMD64 for the EC view → STATUS_NOT_SUPPORTED (0xc00000bb) loading ntdll for any
# x86_64 process. Unstripped, the hybrid DLLs load and x86_64 runs.
{
...
let
  ...
  hangoverWineSrc = pkgs.fetchFromGitHub {
    owner = "AndreRH";
    repo = "wine";
    rev = hangoverWineRev;
...
in
(pkgs.wineWow64Packages.unstable.override { gstreamerSupport = true; }).overrideAttrs (old: {
  pname = "wine-hangover";
  version = "11.15-arm64ec";
  src = hangoverWineSrc;

  # Add arm64ec to --enable-archs (ARM64X-hybrid core DLLs). nixpkgs' default is aarch64,x86_64,i386.
  configureFlags = map (
    f: if lib.hasPrefix "--enable-archs=" f then "--enable-archs=arm64ec,aarch64,x86_64,i386" else f
  ) old.configureFlags;
